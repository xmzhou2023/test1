...
def sql_query_db(databases,sql,listname):
    ...
    lis = query_db(databases=databases,sql=sql)
...
class AssertMode(PubicMethod):
    ...
    def many_db_assert_element_equal(self,actual_results,
        ...
        try:
            if default != None:
                actual = self.get_element_handling(element=actual_results,chioce=choice,get_attribute=get_attribute)
...
if __name__ == '__main__':
    sql=sql_query_db(db_ipm_config_uat,'SELECT display_name,actual_value FROM coding_rule_attributes WHERE code_rule_bid in (SELECT bid FROM code_rule WHERE `name`="整机物料描述CN");','display_name')
    sql1 = sql_query_db(db_ipm_config_uat,
                       'SELECT display_name,actual_value FROM coding_rule_attributes WHERE code_rule_bid in (SELECT bid FROM code_rule WHERE `name`="整机物料描述CN");',
                       'actual_value')
    sql3=sql_query_db(db_ipm_config_uat,'SELECT JSON_EXTRACT(`constraint`,"$.type") FROM object_attr WHERE `explain`="项目名" and obj_bid ="20100001" order by updated_time DESC LIMIT 1;','JSON_EXTRACT(`constraint`,"$.type")')
    dic2= {sql[i]:sql1[i] for i in range(len(sql))}
    print(sql3)
    def test2(name):
        if name=='品牌':
            sql=sql3
            sql_query_db(db_ipm_config_uat,)


    # generatelist(databases_one=db_ipm_config_uat,
    #              sql_one="SELECT node_bid FROM flow_mat_node WHERE flow_type='MOBILE'AND state_code='enable'",
    #              listname_one='node_bid',
    #              databases_two=hulk_workflow_transfer_uat,
    #              sql_two='SELECT field_name FROM flow_field WHERE field_code',
