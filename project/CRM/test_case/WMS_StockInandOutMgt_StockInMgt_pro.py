...
from project.CRM.page_object.WMS_StockInandOutMgt_StockInMgt import *
"""
    用例等级说明:
...
class TestStockInSearch:
    @allure.story("查询当月Allocation In类型的数据")  # 场景名称
    @allure.title("根据type和时间筛选值进行查询")  # 用例名称
    @allure.description("type选择allocation in,时间默认为当前月，其余条件为空进行查询")
    @allure.severity("blocker")  # 用例等级
    @pytest.mark.smoke  # 用例标记
    def test_001_001(self, drivers):  # 用例名称取名规范'test+场景编号+用例编号'
        num = StockInSearch(drivers)
        num.stockinsearch(scope='allocation')
