...
class MonSupResources(Base):
    ...
    @allure.step("返回提报进行数量 用做断言")
    def get_listText(self,brand):
        dic_brand = {"TECNO":"1","itel":"2","Infinix":"3"}
        try:
            a = self.find_element(user['获取某机型提报机型数'],dic_brand[brand])
            b = a.get_attribute("innerHTML")
            c = re.findall("<div>(.*?)</div>", b)[0]
            return c
        except Exception:
            a = self.find_element(user['获取某机型提报机型数'],"1")
            b = a.get_attribute("innerHTML")
            c = re.findall("<div>(.*?)</div>", b)[0]
            return c
    ...
    def SQL_assert(self, year= None, month= None, brand= None):
        ...
        if month is not None:
            num = user.query_db(
                "select model_qty from md_resource_maintain where fyear = '{}' and fmonth = '{}' and brand_code = '{}';".format(
                    year,month, brand))
            for item in num:
                for k, v in item.items():
                    logging.info("获取数据库 提报机型数量:{}".format(v))
                    return str(v)
        else:
            num = user.query_db(
                "select count(*) from md_resource_maintain where fyear = '{}' and fmonth > 0 ;".format(
                    year))
            for item in num:
                for k, v in item.items():
                    logging.info("获取数据库 提报机型数量:{}".format(v))
    ...
    def check_details(self):
        self.is_click(user['列表指定行 查看按钮'],"1")
        logging.info("点击查看按钮")
        sleep(2)
