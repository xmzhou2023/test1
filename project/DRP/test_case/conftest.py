...
def __init__(drivers, env_name):
    ...
    user = Login(drivers)
    user.login(drivers, ini.url, account[11]['usernum'], account[11]['passwd'])
    user = DomAssert(drivers)
    user.assert_url("{}/dashboard".format(ini.url))
    user = SQLAssert(pro_name, pro_env)
    user.assert_sql(word=account[11]['username'], sql='select name_zh from uc_user where enable_flag=1')
