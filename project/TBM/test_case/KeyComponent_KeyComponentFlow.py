...
class TestTheProcessOfExaminationAndApproval:
    ...
    def test_002_001(self, drivers, Component_API):
        ...
        user.enter_oneworks_edit(Component_API[0], '摄像头+闪光灯')
    ...
    def test_002_002(self, drivers, Component_API):
        user = KeyComponentsFlow(drivers)
        user.refresh_webpage()
        user.enter_oneworks_edit(Component_API[0], '硬件电子料-基带')
    ...
    def test_002_003(self, drivers, Component_API):
        user = KeyComponentsFlow(drivers)
        user.refresh_webpage()
        user.onework_flowone(Component_API[0])
        user.onework_flowtwo(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '标准化代表')
    ...
    def test_002_004(self, drivers, Component_API):
        ...
        user.onework_flowone(Component_API[0])
        user.onework_flowtwo(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '采购代表')
        user.click_onework_checkbox()
        user.click_onework_onepress()
    ...
    def test_002_005(self, drivers, Component_API):
        ...
        user.onework_flowone(Component_API[0])
        user.onework_flowtwo(Component_API[0])
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '资源商务评估')
        user.click_onework_unfold('摄像头+闪光灯')
        user.click_onework_module('CTP')
        user.click_onework_material_pending_code('CTP(1供)')
    ...
    def test_002_006(self, drivers, Component_API):
        ...
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '采购执行评估')
        user.click_onework_unfold('摄像头+闪光灯')
        user.click_onework_module('CTP')
        user.click_onework_material_pending_code('CTP(1供)')
    ...
    def test_002_007(self, drivers, Component_API):
        ...
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '采购PTC评估')
        user.click_onework_unfold('摄像头+闪光灯')
        user.click_onework_module('CTP')
        user.click_onework_material_pending_code('CTP(1供)')
    ...
    def test_002_008(self, drivers, Component_API):
        ...
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '采购SQM评估')
        user.click_onework_unfold('摄像头+闪光灯')
        user.click_onework_module('CTP')
        user.click_onework_material_pending_code('CTP(1供)')
    ...
    @allure.description("标准化部评估节点，（所有前面填过信息的这里都可以填标准化评估的一部分）勾选认证状态、评审结论（选通过或者选建议修改，填原因及修改建议），填写原因及修改建议，点击同意，有必填不填时有提示，点击确定")
    @allure.severity("normal")  # 用例等级
    @pytest.mark.UT  # 用例标记
    def test_002_009(self, drivers, Component_API):
        ...
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.enter_oneworks_edit(Component_API[0], '标准化部评估')
        user.click_onework_unfold('摄像头+闪光灯')
        user.click_onework_module('CTP')
        user.click_onework_material_pending_code('CTP(1供)')
        user.input_onework_standardized_evaluation('认证状态', '认证通过')
        user.input_onework_standardized_evaluation('原因及修改建议', '原因及修改建议test')
        user.click_onework_unfold('硬件电子料-基带')
        user.click_onework_module('CPU')
        user.click_onework_material_pending_code('CPU(1供)')
        user.input_onework_standardized_evaluation('认证状态', '认证通过')
    ...
    def test_002_010(self, drivers, KeyComponents_SQL, Component_API):
        ...
        user.onework_flowthree(Component_API[0])
        user.onework_flowfour(Component_API[0])
        user.onework_flowfive(Component_API[0])
        user.onework_flowsix(Component_API[0])
        user.onework_ptc(Component_API[0])
        user.onework_sqm(Component_API[0])
        user.onework_standardized_evaluation(Component_API[0])
        user.assert_my_application_node(Component_API[0], '审批抄送', True)
        sleep(60)
