...
class TestTheProcessOfExaminationAndApproval:
    ...
    @allure.severity("normal")  # 用例等级
    @pytest.mark.smoke  # 用例标记
    def test_002_009(self, drivers, KeyDevice_SQL, KeyDevice_API):
        user = KeyComponentsFlow(drivers)
        user.refresh_webpage()
        user.onework_flowone(KeyDevice_API[0])
        user.onework_flowtwo(KeyDevice_API[0])
        user.onework_flowthree(KeyDevice_API[0])
        user.onework_flowfour(KeyDevice_API[0])
        user.onework_flowfive(KeyDevice_API[0])
        user.onework_flowsix(KeyDevice_API[0])
        user.onework_ptc(KeyDevice_API[0])
        user.onework_sqm(KeyDevice_API[0])
        user.onework_standardized_evaluation(KeyDevice_API[0])
        user.assert_my_application_node(KeyDevice_API[0], '审批抄送', True)
        sleep(60)
        user.assert_my_application_flow(KeyDevice_API[0], '审批完成')
