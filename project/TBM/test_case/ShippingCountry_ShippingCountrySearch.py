...
class TestCreateProcess:
    ...
    def test_001_002(self, drivers): # 用例名称取名规范'test+场景编号+用例编号'
        ...
        user.refresh_webpage_click_menu()
        user.input_condition('品牌', 'Infinix')
        user.input_condition('项目名称', '项目名称test2022-07-14-14:44:47')
        user.click_search()
        user.click_checkbox('项目名称test2022-07-14-14:44:47')
        user.click_change('变更产品')
        user.click_oneworks_product_definition_info_edit('项目名称test2022-07-14-14:44:47')
        ...
        process_code = user.get_info('项目名称test2022-07-14-14:44:47')[2]
...
class TestTheProcessOfExaminationAndApproval:
    ...
    def test_002_003(self, drivers):  # 用例名称取名规范'test+场景编号+用例编号'
        ...
        user.click_oneworks_product_definition_info_edit('项目名称test2022-07-14-14:44:47')
        user.edit_oneworks_product_definition_info('全球版本', '版本2')
        user.edit_oneworks_product_definition_info('市场名称', '修改市场名称')
        user.edit_oneworks_product_definition_info('项目名称', '修改项目名称')
        user.edit_oneworks_product_definition_info('MEMORY', '64+8')
        user.edit_oneworks_product_definition_info('BANDSTRATEGY', '公开市场')
        user.edit_oneworks_product_definition_info('项目经理', '李小素')
        ...
        user.click_oneworks_product_definition_info_edit('修改项目名称')
        querytime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        user.edit_oneworks_product_definition_info('全球版本', '版本3')
    ...
    def test_002_004(self, drivers):  # 用例名称取名规范'test+场景编号+用例编号'
        ...
        user.edit_oneworks_product_definition_info('BANDSTRATEGY', '公开市场')
        user.edit_oneworks_product_definition_info('项目经理', '李小素')
        user.edit_oneworks_product_definition_info('aaa', '4G')
        user.edit_oneworks_product_definition_info('bbb', 'MT6580P')
        user.click_oneworks_product_definition_info_confirm()
        user.select_signatory('汇签人员', '李小素')
        user.click_add_submit()
        user.assert_toast()
        ...
        user.assert_my_todo_node(process_code, '产品经理修改', True)
        user.enter_oneworks_edit(process_code)
        user.click_oneworks_product_definition_info_edit(f'修改项目名称{querytime}')
        user.edit_oneworks_product_definition_info('全球版本', '版本3')
        querytime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        user.edit_oneworks_product_definition_info('市场名称', f'修改市场名称test{querytime}')
        user.edit_oneworks_product_definition_info('项目名称', f'修改项目名称test{querytime}')
        user.edit_oneworks_product_definition_info('MEMORY', '256+8')
        user.edit_oneworks_product_definition_info('BANDSTRATEGY', '印度市场')
        user.edit_oneworks_product_definition_info('aaa', '2G')
        user.edit_oneworks_product_definition_info('bbb', 'MT6762D')
    ...
    def test_002_005(self, drivers):  # 用例名称取名规范'test+场景编号+用例编号'
        ...
        user.refresh_webpage_click_menu()
        user.input_condition('品牌', 'Infinix')
        user.input_condition('项目名称', '项目名称test2022-07-12-14:22:55')
        user.click_search()
        user.click_checkbox('项目名称test2022-07-12-14:22:55')
        user.click_change('变更产品')
        querytime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        user.click_oneworks_product_definition_info_edit('项目名称test2022-07-12-14:22:55')
        user.edit_oneworks_product_definition_info('全球版本', '版本2')
        user.edit_oneworks_product_definition_info('市场名称', f'修改市场名称{querytime}')
        user.edit_oneworks_product_definition_info('项目名称', f'修改项目名称{querytime}')
        user.edit_oneworks_product_definition_info('MEMORY', '64+8')
        ...
        user.click_add_submit()
        user.assert_toast()
        process_code = user.get_info('项目名称test2022-07-12-14:22:55')[2]
        user.onework_agree_flow(process_code, '产品部管理员审核')
        user.onework_agree_flow(process_code, '产品部汇签')
        ...
        document_status = user.get_info('项目名称test2022-07-12-14:22:55')[6]
        ValueAssert.value_assert_equal(document_status, '审批通过')
        user.refresh_webpage_click_menu()
        user.input_condition('品牌', 'Infinix')
        user.input_condition('项目名称', f'修改项目名称{querytime}')
        user.click_search()
        user.click_checkbox(f'修改项目名称{querytime}')
        user.click_change('变更产品')
        user.click_oneworks_product_definition_info_edit(f'修改项目名称{querytime}')
        user.edit_oneworks_product_definition_info('全球版本', '版本2')
        user.edit_oneworks_product_definition_info('市场名称', '市场名称test2022-07-12-14:22:55')
        user.edit_oneworks_product_definition_info('项目名称', '项目名称test2022-07-12-14:22:55')
        user.edit_oneworks_product_definition_info('MEMORY', '64+8')
        user.edit_oneworks_product_definition_info('BANDSTRATEGY', '公开市场')
        user.edit_oneworks_product_definition_info('项目经理', '李小素')
        user.edit_oneworks_product_definition_info('aaa', '4G')
        ...
        user.click_add_submit()
        user.assert_toast()
        process_code = user.get_info(f'修改项目名称{querytime}')[2]
        user.onework_agree_flow(process_code, '产品部管理员审核')
        user.onework_agree_flow(process_code, '产品部汇签')
        user.onework_agree_flow(process_code, '产品经理修改')
        user.onework_agree_flow(process_code, '项目经理审批')
