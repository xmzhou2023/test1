...
class TestTheProcessOfExaminationAndApproval:
    ...
    @allure.story("流程审批")  # 场景名称
    @allure.title("项目经理审批复核审核成功")  # 用例名称
    @allure.description("项目经理审批：点击同意，提示请求成功")
    @allure.severity("normal")  # 用例等级
    @pytest.mark.smoke  # 用例标记
    ...
    @allure.description("用例：流程未配置区域：  发起流程 EE1更改为 ●，走完流程检查EE1是否为 ●；"
                        "发起流程 乍得更改为 ●，走完流程检查乍得是否为 ●；发起流程 中国更改为 ●，走完流程检查中国是否为 ●；")
    @allure.severity("normal")  # 用例等级
    @pytest.mark.smoke  # 用例标记
...
class TestProcessApprovalExceptionScenario:
    ...
    @allure.title("第1行产品国家【XXX】已经存在流程中单据")  # 用例名称
    @allure.description("流程未配置区域：发起变更国家流程，东亚EE1更改为●；再次发起变更国家流程，东亚EE1更改为●，点击提交提示重复")
    @allure.severity("normal")  # 用例等级
    @pytest.mark.smoke  # 用例标记
    ...
    @allure.description("流程未配置区域：发起变更国家流程，东亚EE1更改为●；再次发起变更国家流程，中国更改为●，"
