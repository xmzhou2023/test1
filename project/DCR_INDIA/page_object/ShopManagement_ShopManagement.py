...
class ShopManagementPage(Base):
    ...
    def assert_Query_result(self, header, content):
        ...
        """
        DomAssert(self.driver).assert_search_result(user['menu表格字段'], user['表格内容'], header, content, sc_element=user['滚动条'],
    ...
    def assert_search_result(self, header, content):
        ...
        if header == 'Shop' or header == 'Retail Customer':
            ...
            for i in range(3):
                assert_result = False
                column = self.get_table_info(user['menu表格字段'], f'{header} {3 - i}', index=1, sc_element=user['滚动条'],
                                             h_element=user['表头文本'])
