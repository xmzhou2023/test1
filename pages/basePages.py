...
'''
TestBasePage类是POM中的基类，主要用于提供常用的函数，为页面对象类进行服务；
selenuim
常用函数：元素定位，输入，点击，访问url,等待，关闭等
'''
...
class TestBasePages:
    ...
    # 构造函数
    # def __init__(self, browser_name='chrome', target_url=''):
    def __init__(self):
        pass
        # self.logger = MyLog().getLog()
        # self.driver = driver

    # def initial_driver(self, browser_name='chrome', target_url=''):
        '''
        虚构driver对象
        :param browser_name:
        :param target_url:
    ...
    #     :return:None
    #     """
    #     # self.driver = Pages.initial_driver(self)
    ...
    #
    # 查找一个元素element
    def find_locat(self, loc, model=None):
        logging.info(f'查找"{model}"元素，元素定位:{loc}')
        try:
            # 元组：*loc
            return self.driver.find_element(*loc)
        except NoSuchElementException:
            logging.exception(f'查找"{model}"元素失败,定位方式:{loc}')
            # 截图
            # self.save_webImgs(f"查找元素[{model}]异常")
            raise

    # 查找元素elements
    def find_locats(self, loc, model=None):
        logging.info(f'查找"{model}"元素集，元素定位:{loc}')
        try:
            return self.driver.find_elements(*loc)
        except NoSuchElementException:
            logging.exception(f'查找"{model}"元素集失败,定位方式:{loc}')
            # 截图
    ...
    def input_text(self, loc, text, model=None):
        # 查找元素
        ele = self.find_locat(loc, model)
        # 输入操作
        logging.info(f'在"{model}"输入"{text}",元素定位:{loc}')
        try:
            ele.send_keys(text)
        except:
            logging.exception(f'"{model}"输入操作失败!')
            # 截图
    ...
    def click_element(self, loc, model=None):
        ...
        ele = self.find_locat(loc, model)
        # 点击操作
        logging.info(f'点击"{model}",元素定位:{loc}')
        try:
            ele.click()
        except:
            logging.exception(f'"{model}"点击失败')
            # 截图
